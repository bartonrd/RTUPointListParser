...
namespace RTUPointlistParse
{
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            // Verify extracted point names if Example1 is being processed
            if (inputFolder.Contains("Example1"))
            {
                Console.WriteLine();
                Console.WriteLine("Verifying point names...");
                Console.WriteLine("========================");
                VerifyPointNames(allAnalogRows);
            }
        ...
        /// IMPORTANT: Only filters exact matches - "SPARE A 74" and similar names are kept
        /// </summary>
        ...
        /// <summary>
        /// Verify extracted point names against the expected list for Example1
        /// </summary>
        private static void VerifyPointNames(List<TableRow> analogRows)
        {
            ...
            // Extract actual point names (normalize whitespace)
            var actualNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (var row in analogRows)
            {
                if (row.Columns.Count > 1)
                {
                    string name = WhitespaceNormalizePattern.Replace(row.Columns[1].Trim(), " ");
                    actualNames.Add(name);
                }
            }

            // Find missing and extra names
            var missingNames = expectedNames.Except(actualNames, StringComparer.OrdinalIgnoreCase).ToList();
            var extraNames = actualNames.Except(expectedNames, StringComparer.OrdinalIgnoreCase).ToList();

            Console.WriteLine($"  Expected: {expectedNames.Count} point names");
            Console.WriteLine($"  Extracted: {actualNames.Count} point names");

            if (missingNames.Count > 0)
            {
                Console.WriteLine($"\n  Missing {missingNames.Count} point name(s):");
            ...
            {
                Console.WriteLine($"\n  Extra {extraNames.Count} point name(s):");
            ...
            {
                Console.WriteLine("  ✓ All point names match perfectly!");
            }
        }
